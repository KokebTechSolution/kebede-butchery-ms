...
# --- Lookup Tables (Rest of your models as provided, unchanged from last response) ---
...
class Product(models.Model):
    ...
    def get_conversion_factor(self, from_unit, to_unit):
        """
        Retrieves the conversion factor for this product between two given units.
        Returns 1.0 if from_unit and to_unit are the same.
        Raises ValueError if conversion path not found.
        This method will use the new ProductMeasurement model for lookups.
        """
        if from_unit == to_unit:
            return Decimal('1.0')

        # Try direct conversion using ProductMeasurement
        try:
            ...
            pass # Try reverse or via base unit
        ...
        try:
            ...
            pass # Try via base unit

        # Try converting via product's base unit (if neither direct nor reverse exists)
        if from_unit != self.base_unit and to_unit != self.base_unit:
            try:
                ...
                pass # Conversion path not found
...
class BarmanStock(models.Model):
    ...
    def __str__(self):
        return f"{self.bartender.username} — {self.product.name} ({self.quantity_in_base_units} {self.product.base_unit.unit_name})"
    ...
    # --- NEW: Methods for displaying quantity in other units ---

    def get_quantity_in_unit(self, target_unit):
        """
        Calculates the barman's current stock quantity in a specified target unit.
        Returns Decimal or None if conversion is not possible/found.
        """
        if not isinstance(target_unit, ProductUnit):
            raise TypeError("target_unit must be an instance of ProductUnit.")

        try:
            # We need to convert from our base_unit to the target_unit.
            # The get_conversion_factor method finds how many 'to_unit' are in one 'from_unit'.
            # So, to go from base_unit to target_unit, we need that factor.
            conversion_factor = self.product.get_conversion_factor(self.product.base_unit, target_unit)

            # Multiply quantity_in_base_units by this factor to get quantity in target_unit
            # Example: 2 bottles (base) * 20 (shots/bottle) = 40 shots
            # Example: 48 bottles (base) / 24 (bottles/carton) = 2 cartons  <-- This means conversion_factor should be 1/24 here.
            # get_conversion_factor already handles this directionality correctly.
            return self.quantity_in_base_units * conversion_factor
        except ValueError as e:
            # This means no conversion path was found for Product.get_conversion_factor
            # You might want to log this or return a specific error/value.
            print(f"Warning: No valid conversion path for Product '{self.product.name}' from base unit '{self.product.base_unit.unit_name}' to target unit '{target_unit.unit_name}': {e}")
            return None # Or Decimal('0.00') depending on desired behavior

    @property
    def display_stock_summary(self):
        """
        Returns a human-readable summary of the barman's stock,
        showing quantity in base unit and other common relevant units.
        """
        summary_parts = []
        # Always show quantity in base unit
        summary_parts.append(
            f"{self.quantity_in_base_units.quantize(Decimal('0.01'))} "
            f"{self.product.base_unit.abbreviation or self.product.base_unit.unit_name}"
        )

        try:
            # Attempt to get common display units for this product
            # 1. Carton quantity (if base unit is smaller than carton, e.g., bottle)
            carton_unit = ProductUnit.objects.get(unit_name='carton')
            if self.product.base_unit != carton_unit: # Only convert if base unit isn't already carton
                carton_qty = self.get_quantity_in_unit(carton_unit)
                if carton_qty is not None and carton_qty >= Decimal('1.00'): # Show if at least one full carton
                    summary_parts.append(f"({carton_qty.quantize(Decimal('0.01'))} {carton_unit.abbreviation or carton_unit.unit_name})")
        except ProductUnit.DoesNotExist:
            pass # No 'carton' unit defined in system

        try:
            # 2. Shot/Piece quantity (if product is liquid or commonly sold in smaller pieces)
            # Find a default sales unit if one exists, otherwise try 'shot'
            default_sales_conversion = self.product.measurements.filter(
            ...
            elif self.product.base_unit.is_liquid_unit: # Fallback to shot if liquid
                target_display_unit = ProductUnit.objects.get(unit_name='shot')
            elif self.product.category.item_type.type_name == 'Food': # Example: try 'piece' for food items
                target_display_unit = ProductUnit.objects.get(unit_name='piece')


            if target_display_unit and target_display_unit != self.product.base_unit: # Avoid redundant display
                display_qty = self.get_quantity_in_unit(target_display_unit)
                if display_qty is not None and display_qty >= Decimal('0.01'): # Show if positive quantity
            ...
            pass # Specific unit (like 'shot' or 'piece') not defined

        return " ".join(summary_parts)


# --- Transaction & Request Models (Unchanged) ---
