...
class Product(models.Model):
    ...
    def get_smallest_unit(self):
        """
        Returns the ProductUnit instance representing the smallest unit for this product.
        The smallest unit is the one that is only a to_unit in measurements, never a from_unit.
        """
        ...
        if not smallest_unit_ids:
            return None
        # Return the first found smallest unit (usually only one)
        return ProductUnit.objects.get(id=smallest_unit_ids[0])

    def get_conversion_factor_to_smallest(self, from_unit):
        """
        Returns the total conversion factor from from_unit to the smallest unit for this product.
        Chains through all measurements as needed.
        """
...
class Stock(models.Model):
    ...
    def update_running_out_status(self):
        # Only compare if both are real numbers (not F/CombinedExpression)
        ...
        if isinstance(self.quantity_in_base_units, (CombinedExpression, F)) or isinstance(self.minimum_threshold_base_units, (CombinedExpression, F)):
            return  # Skip, will be handled after DB refresh
    ...
    def save(self, *args, **kwargs):
        # Allow bypassing full_clean when saving with F() expressions
...
class BarmanStock(models.Model):
    ...
    def display_stock_summary(self):
        ...
        try:
            ...
            if self.stock.product.base_unit != carton_unit: # Only convert if base unit isn't already carton
        ...
        try:
            ...
            elif self.stock.product.base_unit.is_liquid_unit: # Fallback to shot if liquid
                target_display_unit = ProductUnit.objects.get(unit_name='shot')
            elif self.stock.product.category.item_type.type_name == 'Food': # Example: try 'piece' for food items
                target_display_unit = ProductUnit.objects.get(unit_name='piece')


            if target_display_unit and target_display_unit != self.stock.product.base_unit: # Avoid redundant display
