...
const MenuTable = ({ refreshFlag, onEdit, onDelete, loading, searchTerm = '' }) => {
    ...
    return (
        <div className="p-4">
            {/* Category Filter and Discount Section */}
            ...
            {filteredMenuItems.length === 0 ? (
                <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-lg font-medium text-gray-900">
                        {searchTerm ? t('no_matching_menu_items') : t('no_menu_items_found')}
                    </h3>
                    <p className="mt-1 text-sm text-gray-500">
                        {searchTerm ? t('try_different_search') : t('no_menu_items_yet')}
                    </p>
                </div>
            ) : isMobile ? (
                // Mobile card view
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredMenuItems.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <span className="font-semibold text-orange-600">ETB {item.price}</span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <button 
                                            onClick={() => handleEdit(item)} 
                                            className="text-blue-600 hover:text-blue-900 bg-blue-100 hover:bg-blue-200 px-3 py-1 rounded text-sm"
                                        >
                                            {t('edit')}
                                        </button>
                                        <button 
